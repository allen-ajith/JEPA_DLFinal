...
class MediumEncoder(nn.Module):
    def __init__(self, repr_dim=256):
        super().__init__()
        self.convnet = nn.Sequential(
            # First conv layer: 2 channels -> 64 channels
            nn.Conv2d(2, 64, kernel_size=4, stride=2, padding=1),  # 32x32
            nn.BatchNorm2d(64),
            nn.ReLU(True),
            
            # Second conv layer: 64 channels -> 128 channels
...
class MediumPredictor(nn.Module):
    def __init__(self, repr_dim=256, action_dim=2, hidden_dim=256):
        super().__init__()
        # Two-layer predictor with matching complexity to encoder
        self.net = nn.Sequential(
            # First layer
...
class MediumJEPAModel(nn.Module):
    def __init__(self, repr_dim=256):
        super().__init__()
        self.repr_dim = repr_dim
        self.encoder = MediumEncoder(repr_dim=repr_dim)
        self.predictor = MediumPredictor(repr_dim=repr_dim)
